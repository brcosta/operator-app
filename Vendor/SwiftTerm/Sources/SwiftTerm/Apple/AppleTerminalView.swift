...
extension TerminalView {
    ...
    )
    {
        ...
        // Operator smooth scrolling: map the invalidated view rect back into the translated
        // content coordinate space. `floor` is essential for the partially visible row above
        // the viewport; integer conversion truncates a negative fraction to zero.
        let contentDirtyRect = dirtyRect.offsetBy(dx: 0, dy: -verticalOffset)
        let firstRow = displayBuffer.yDisp
            + Int(floor((boundsMaxY - contentDirtyRect.maxY) / cellHeight))
        let lastRow = displayBuffer.yDisp
            + Int(floor((boundsMaxY - contentDirtyRect.minY) / cellHeight))
        #endif
    ...
    func updateCursorPosition()
    {
        ...
        #if os(macOS)
        let viewportTop = CGFloat(buffer.yDisp) - smoothScrollOffset / cellDimension.height
        #else
        let viewportTop = CGFloat(buffer.yDisp)
        #endif
        let caretTop = CGFloat(vy)
        let caretBottom = caretTop + 1
        if caretTop >= viewportTop + CGFloat(buffer.rows) || caretBottom <= viewportTop {
        ...
        let lineOrigin = CGPoint(x: 0, y: frame.height - offset - smoothScrollOffset)
        #endif
    ...
    public var scrollPosition: Double {
        get {
            ...
            #if os(macOS)
            let effectiveRow = CGFloat(displayBuffer.yDisp)
                - smoothScrollOffset / max(cellDimension.height, 1)
            #else
            let effectiveRow = CGFloat(displayBuffer.yDisp)
            #endif
            if effectiveRow <= 0 {
            ...
            if effectiveRow >= CGFloat(maxScrollback) {
                return 1
            }
            
            return Double(effectiveRow) / Double(maxScrollback)
        }
    }
