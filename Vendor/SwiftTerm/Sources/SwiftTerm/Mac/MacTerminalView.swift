...
/// A continuous terminal viewport represented by a logical buffer row and a sub-row offset.
///
/// Positive offsets reveal the preceding row at the top of the viewport; negative offsets
/// reveal the following row at the bottom.
public struct TerminalSmoothScrollPosition: Equatable {
    public private(set) var row: Int
    public private(set) var offset: CGFloat

    public init(row: Int, offset: CGFloat = 0) {
        self.row = row
    ...
    public mutating func consume(
        ...
        guard deltaPoints.isFinite, cellHeight.isFinite, cellHeight > 0 else { return }
        let lower = min(minimumRow, maximumRow)
        let upper = max(minimumRow, maximumRow)
        let currentRow = min(max(row, lower), upper)
        let currentOffset = offset.isFinite ? offset : 0
        let currentPosition = CGFloat(currentRow) * cellHeight - currentOffset
        let target = min(
            max(currentPosition - deltaPoints, CGFloat(lower) * cellHeight),
            CGFloat(upper) * cellHeight
        )
        let targetInRows = target / cellHeight
        let nearestRow = targetInRows.rounded()

        if abs(targetInRows - nearestRow) < 0.000_001 {
            row = min(max(Int(nearestRow), lower), upper)
            offset = 0
        } else if deltaPoints > 0 {
            row = min(max(Int(ceil(targetInRows)), lower), upper)
            offset = CGFloat(row) * cellHeight - target
        } else if deltaPoints < 0 {
            row = min(max(Int(floor(targetInRows)), lower), upper)
            offset = CGFloat(row) * cellHeight - target
        } else {
            row = currentRow
    ...
    public func bufferRow(
        forViewY viewY: CGFloat,
        viewHeight: CGFloat,
        cellHeight: CGFloat
    ) -> Int {
        guard viewY.isFinite, viewHeight.isFinite, cellHeight.isFinite, cellHeight > 0 else {
            return row
        }
        let relativeRow = Int(floor((viewHeight - viewY - offset) / cellHeight))
...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Operator smooth scrolling: fractional viewport displacement in points.
    var smoothScrollOffset: CGFloat = 0
    var isCommittingSmoothScroll = false
    private var pendingSmoothWheelPoints: CGFloat = 0
    private var smoothWheelTimer: Timer?
    private var smoothScrollAccessibilityWorkItem: DispatchWorkItem?
#if canImport(MetalKit)
    ...
    deinit {
        ...
        smoothWheelTimer?.invalidate()
        smoothScrollAccessibilityWorkItem?.cancel()
    ...
    override public func draw (_ dirtyRect: NSRect) {
        ...
        drawTerminalContents(
            ...
            bufferOffset: terminal.displayBuffer.yDisp,
            verticalOffset: -smoothScrollOffset
        )
    }
    ...
    func calculateMouseHit (at point: CGPoint) -> (grid: Position, pixels: Position)
    {
        ...
        let position = TerminalSmoothScrollPosition(
            row: displayBuffer.yDisp,
            offset: smoothScrollOffset
        )
        let bufferRow = position.bufferRow(
            forViewY: point.y,
    ...
    /// Fractional input retained only when Reduce Motion requests line-at-a-time scrolling.
    private var reducedMotionScrollAccumulator: CGFloat = 0
    ...
    public override func scrollWheel(with event: NSEvent) {
        ...
        if allowMouseReporting && !shiftBypassesMouseReporting(for: event) && terminal.mouseMode != .off {
            resetSmoothScrollOffset()
            let lines = wholeLineScrollAmount(
                for: event,
        ...
        } else if !NSWorkspace.shared.accessibilityDisplayShouldReduceMotion && !isUsingMetalRenderer {
            reducedMotionScrollAccumulator = 0
            if event.hasPreciseScrollingDeltas {
                cancelSmoothWheelAnimation()
                applySmoothScroll(deltaPoints: scaledDelta)
            } else {
                let rounded = scaledDelta.rounded()
                let lineDelta = rounded == 0
                    ? (event.scrollingDeltaY > 0 ? CGFloat(1) : CGFloat(-1))
                    : rounded
                enqueueSmoothWheelScroll(deltaPoints: lineDelta * cellHeight)
            }
        } else {
            resetSmoothScrollOffset()
            let lines = wholeLineScrollAmount(
    ...
    private func wholeLineScrollAmount(
        for event: NSEvent,
        scaledDelta: CGFloat,
        cellHeight: CGFloat
    ) -> Int {
        if event.hasPreciseScrollingDeltas {
            reducedMotionScrollAccumulator += scaledDelta
            let lines = Int(reducedMotionScrollAccumulator / cellHeight)
            reducedMotionScrollAccumulator -= CGFloat(lines) * cellHeight
            return lines
        }
        reducedMotionScrollAccumulator = 0
    ...
    private func enqueueSmoothWheelScroll(deltaPoints: CGFloat) {
        guard deltaPoints.isFinite, deltaPoints != 0 else { return }
        pendingSmoothWheelPoints += deltaPoints
        guard smoothWheelTimer == nil else { return }
        ...
        let timer = Timer(timeInterval: 1.0 / 120.0, repeats: true) { [weak self] timer in
            ...
            self.advanceSmoothWheelAnimation()
        }
        RunLoop.main.add(timer, forMode: .common)
        smoothWheelTimer = timer
    }

    private func advanceSmoothWheelAnimation() {
        let remaining = pendingSmoothWheelPoints
        guard remaining.isFinite, abs(remaining) >= 0.25 else {
            if remaining.isFinite {
                applySmoothScroll(deltaPoints: remaining)
            }
            cancelSmoothWheelAnimation()
            return
        }
        let stepMagnitude = min(abs(remaining), max(0.75, abs(remaining) * 0.24))
        let step = remaining < 0 ? -stepMagnitude : stepMagnitude
        pendingSmoothWheelPoints -= step
        applySmoothScroll(deltaPoints: step)
    }

    private func applySmoothScroll(deltaPoints: CGFloat) {
        let displayBuffer = terminal.displayBuffer
        let maximumRow = max(0, displayBuffer.lines.count - displayBuffer.rows)
        ...
        let previousOffset = smoothScrollOffset
        var position = TerminalSmoothScrollPosition(row: previousRow, offset: previousOffset)
        position.consume(
            deltaPoints: deltaPoints,
            cellHeight: cellDimension.height,
            maximumRow: maximumRow
        )
        guard position.row != previousRow || position.offset != previousOffset else { return }

        smoothScrollOffset = position.offset
        if position.row != previousRow {
            isCommittingSmoothScroll = true
            scrollTo(row: position.row, notifyAccessibility: false)
            isCommittingSmoothScroll = false
        } else {
            let effectiveRow = CGFloat(position.row) - position.offset / cellDimension.height
            let isUserScrolling = effectiveRow < CGFloat(maximumRow)
        ...
        updateCursorPosition()
        setNeedsDisplay(bounds)
        scheduleSmoothScrollAccessibilityUpdate()
    }

    private func cancelSmoothWheelAnimation() {
        smoothWheelTimer?.invalidate()
        smoothWheelTimer = nil
        pendingSmoothWheelPoints = 0
    }

    private func scheduleSmoothScrollAccessibilityUpdate() {
        smoothScrollAccessibilityWorkItem?.cancel()
        ...
        smoothScrollAccessibilityWorkItem = workItem
    ...
    func resetSmoothScrollOffset() {
        cancelSmoothWheelAnimation()
        smoothScrollAccessibilityWorkItem?.cancel()
        smoothScrollAccessibilityWorkItem = nil
        guard smoothScrollOffset != 0 else { return }
